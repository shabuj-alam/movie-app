...
export const fetchMovies = async({query}: {query: string}) => {
    const endpoint = (query !== '')
        ? `${TMDB_CONFIG.BASE_URL}/search/movie?sort_by=rating.desc&query=${encodeURIComponent(query)}`
...
export const fetchMovieDetails = async(movieId: string): Promise<MovieDetails| any> => {
    try {
